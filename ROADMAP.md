...
| 機能 | 説明 |
|------|------|
| **`Ctrl+R =`** | 式レジスタ（計算結果を挿入） |
| **`Ctrl+K {a}{b}`** | ダイグラフ文字の入力 |
...
| 機能 | 説明 |
|------|------|
| **`:bufdo` / `:windo` / `:tabdo`** | 全バッファ/ウィンドウ/タブにコマンドを適用 |
| **`:abbreviate` / `:iab`** | インサートモードの略語展開 |
| **`:digraphs`** | 使用可能なダイグラフ一覧を表示 |
| **`q:` / `q/` / `q?`** | コマンドライン/検索履歴ウィンドウ |
| **`:jumps`** | ジャンプリストを表示 |
| **`set colorcolumn={N}`** | 指定列に縦ラインを表示 |
