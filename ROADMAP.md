...
| 項目 | 説明 | 優先度 |
|------|------|-------|
| `H`/`M`/`L` モーション | 固定オフセット値で実装されており実際のスクロールオフセットを参照していない | 高 |
| 大規模ファイルのパフォーマンス | `TextBuffer` は `List<string>` で全行保持。数十万行でレンダリングが重くなる | 中 |
| テストカバレッジ | VisualBlock モード・ExCommand・MotionEngine のテストが薄い | 中 |
| `viminfo` 永続化 | コマンド履歴・レジスタ・マーク・ジャンプリストがセッション終了で消える | 中 |
