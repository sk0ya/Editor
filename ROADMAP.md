...
### 1. テキストオブジェクト拡張
現状 `iw/aw/iW/aW`（単語）のみ実装済み。以下が未実装：
- `i(/a(`, `i)/a)` — 括弧内/含む
- `i{/a{`, `i}/a}` — 波括弧内/含む
- `i[/a[`, `i]/a]` — 角括弧内/含む
- `i"/a"`, `i'/a'`, `` i`/a` `` — クォート内/含む
- `it/at` — HTMLタグ内/含む
- `is/as` — センテンス
- `ip/ap` — パラグラフ（現在は `{/}` の移動のみ）
- **実装箇所:** `CommandParser.ParseMotion` で `s[1]` の許容文字を拡張、`VimEngine.GetTextObjectRange` に各ケース追加

### 2. `:global` コマンド
- `:g/pattern/cmd` — パターン一致行に一括コマンド実行
- `:v/pattern/cmd` (`:g!`) — 不一致行に実行
- 例: `:g/^$/d`（空行削除）、`:g/TODO/yank A`（TODO行をレジスタAに収集）
- **実装箇所:** `ExCommandProcessor.Execute` に `g` / `v` ブランチ追加
...
- `:e <Tab>` — ファイルパス補完
- `:b <Tab>` — バッファ名補完
- `:colorscheme <Tab>` — テーマ名補完
- `set <Tab>` — オプション名補完
- **実装箇所:** `VimEditorControl` のコマンドライン入力ハンドラ、`ExCommandProcessor` に補完候補API追加
